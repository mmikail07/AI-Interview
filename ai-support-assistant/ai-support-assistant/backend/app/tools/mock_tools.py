...
Each tool is a pure function: it takes ``ToolParameters`` and returns a JSON-able
payload built from *static, predefined* data. No tool makes a network call. The
``TOOL_REGISTRY`` maps an :class:`Intent` to its handler so adding a new intent
is a two-line change (register a function here + add the enum/UI mapping).
"""
...
def hotel_tool(p: ToolParameters) -> dict[str, Any]:
    ...
    if not results:  # graceful fallback: unknown city -> show Dubai
        location = "Dubai"
...
# --------------------------------------------------------------------------- #
# Registry (factory pattern) — intent -> (tool_name, handler)
# --------------------------------------------------------------------------- #
...
def run_tool(intent: Intent, params: ToolParameters) -> tuple[str | None, Any]:
    """Execute the tool registered for ``intent``.

    Returns ``(tool_name, payload)``. For ``Intent.other`` (no tool) returns
    ``(None, None)``. Any tool exception is caught and surfaced as a safe payload
    ...
    except Exception as exc:  # defensive: a tool must never 500 the endpoint
