...
Routes
------
POST /chat         -> structured ChatResponse (primary contract)
POST /chat/stream  -> Server-Sent Events: emits stage events then the final
                      payload (bonus: streaming responses)
POST /reset        -> clears a conversation's memory
GET  /health       -> liveness + Ollama reachability

Cross-cutting: CORS for the Flutter client, structured logging, and a per-request
trace id injected via middleware and echoed back in the ``X-Request-ID`` header.
"""
